...
===Ads
====get_ad(method: GET)
#other ad         RPOU(RPOU), RCH(ROTATE CH), FULL(FULL)
#daily hop ad     SP(SPONSOR FULL PG), ROFL(FULL PG - ROTATE), CS(COUPON SCREEN)
#regular hop ad   SP(SPONSOR FULL PG), ROFL(FULL PG - ROTATE), ROPU(POP - UP ROTATE), CS(COUPON SCREEN)
# In case CS-type in params 'hop_id' should be 'id' of hop_task (which user is sharing)
...
assigned: true if user assigned to hop of false if not
    ...
    response:
        success: {"daily_hops"=>[
                                    {
                                        "id"=>35,
                                        "name"=>"broken daily hop",
                                        "time_start"=>"2013-08-06T10:33:57Z",
                                        "time_end"=>"2013-08-06T10:33:57Z",
                                        "code"=>nil,
                                        "price"=>nil,
                                        "jackpot"=>"very big prize",
                                        "link"=>"http://perechin.net",
                                        "daily"=>true,
                                        "close"=>false,
                                        "event"=>nil,
                                        "created_at"=>"2013-08-06T07:34:08Z",
                                        "updated_at"=>"2013-08-06T07:34:08Z",
                                        "assigned"=>false,
                                        "score"=>0,
                                        "logo"=>nil,
                                        "purchased"=>nil,
                                        "zip":"88000, 23233, 11111",
                                        "completed"=>true,
                                        "ask_password"=>nil
                                    }, ...]
                 }
        error: {"errors"=>["Daily hop not found."], "success"=>false, "status"=>406}
        ...
        success: {"hops"=>[
                            {
                                "id"=>1,
                                "name"=>"hop #1",
                                "time_start"=>"01/07/2013 11:35:32",
                                "time_end"=>"01/07/2013 11:35:32",
                                "code"=>"345",
                                "price"=>"2",
                                "jackpot"=>"very big prize",
                                "daily"=>false,
                                ...
                                "close"=>false,
                                "event"=>"new year",
                                "created_at"=>"2013-07-22T08:35:50Z",
                                "updated_at"=>"2013-07-22T08:35:50Z",
                                "assigned"=>true,
                                "score"=>0,
                                "logo"=>"/images/hop_logos/hops/1/HOP_LOGO.jpg?1374831944",
                                "purchased"=>true,
                                "zip":"88000, 23233, 11111",
                                "completed"=>true,
                                "ask_password"=>nil
...
====get hop by id(method: GET)
    ...
    data: {api_key: 123, authentication_token: kNvOIDu1WR0f8MUYjnYdHpXgnYC0dO, hop_id: 1}
    response:
        success: {"hop"=>{"id"=>2,"link"=>"http://perechin.net", "name"=>"admin reg hop #2", "time_start"=>"08/07/2013 09:38:42", "time_end"=>"08/07/2013 09:38:42",
                  "code"=>"34", "price"=>"34", "jackpot"=>"very big prize", "daily"=>false, "close"=>false, "event"=>"34", "created_at"=>"2013-07-23T06:39:03Z",
                  "updated_at"=>"2013-07-23T06:39:03Z", "assigned"=>true, "score"=>0, "logo"=>"/images/hop_logos/hops/1/HOP_LOGO.jpg?1374831944"
                  , "purchased"=>nil, "completed"=>true}}
        error: {"errors"=>["Hop not found."], "success"=>false, "status"=>406}
...
service should be 'twitter', 'facebook', 'google'
    ...
    data: {"api_key":"123","authentication_token":"dtaLlmSO6QcrAUtZsWiY15H2Cv38Ct","hop_task_id":"13","service":"facebook"}
    response:
        success: {"success"=>true, "info"=>"Success.", "status"=>200}
        error: {"errors"=>["Already shared."], "success"=>false, "status"=>406}
        error: {"errors"=>["Cant find feed by."], "success"=>false, "status"=>406}
...
====get hop prizes(method: GET)
    ...
    response:
        ...
        error: {"errors"=>["Hop not found."], "success"=>false, "status"=>406}
====disable hop password(method: POST)
    url: http://localhost:3000/api/hop/disable_password.json
    data: {
            api_key: "123",
            authentication_token: "dtaLlmSO6QcrAUtZsWiY15H2Cv38Ct",
            hop_id: 1,
            password: "p@ssw0rd"
          }
    response:
        success: {"success"=>true, "info"=>"Password successfully disabled.", "status"=>200}
        error: {"errors"=>["Wrong password."], "success"=>false, "status"=>200}
...
====submit users hop task(automatikally assign you to hop)(method: POST)
    ...
    response:
        success: {"success"=>true, "info"=>"Task create!", "status"=>200, "url"=>"/images/user_hop_task_photos/tasks/8/original/PHOTO.jpeg?1380267396"}
...
=====first synchronization - return all unsynchronized messages from last login)
    ...
    response:
        success: {
                  "success"=>true,
                  "last_sync_time"=>"2013-08-06T13:15:36Z",
                  "messages"=>[
                    {"sender_id"=>19,
                     "receiver_id"=>20,
                     "text"=>"message 3",
...
====friend messages history(method: GET)
    ...
    response:
        success: {"messages"=>[
                    {"id"=>1, "sender_id"=>1, "receiver_id"=>2, "text"=>"hello friend", "created_at"=>"2013-09-30T03:54:57-05:00", "time_ago"=>"about 17 hours"},
                    {"id"=>2, "sender_id"=>2, "receiver_id"=>1, "text"=>"hello, friends !!!", "created_at"=>"2013-09-30T03:54:57-05:00", "time_ago"=>"about 17 hours"}
                 ]}
