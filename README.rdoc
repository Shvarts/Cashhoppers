...
====get daily hops (today)(method: GET)
    ...
    response:
        ...
        error: {"errors"=>["Hops not found."], "success"=>false, "status"=>406}
====assign to hop(method: POST)
    url: http://localhost:3000/api/hops/assign.json?api_key=123&authentication_token=lgzYkMy0uUrBisPuuAkhfqVSKbsgp4&hop_id=1
...
====get hop tasks(method: GET)
    ...
    response:
        ...
        error: {"errors"=>["Hop not found."], "success"=>false, "status"=>406}
====submit users hop task(method: POST)
    url: http://localhost:3000/api/task/submit.json
    data: {api_key: 123, authentication_token: H2zCS5AfvAIbS0OdRY6SCJAjM6RM2F, task:{hop_task_id: 1, photo: #<File:1.jpg>}}
    response:
